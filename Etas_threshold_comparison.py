...
#SET SCALE FACTOR IN CONFIG OPTIONS
@export
@strax.takes_config(
    ...
    strax.Option('scale_factor', default=50000),
             help="The scale factor for the hit threshold (thresh = area/scale_factor)"),
    *HITFINDER_OPTIONS,
)
class SplitPeakletsTest(strax.Plugin):
    """
    Split records into:
        -peaklets
        -lone_hits
    ...
    """
    depends_on = ('records')
    provides = 'split_peaklets_test'
    data_kind = 'split_peaklets_test'
    parallel = 'process'
    compressor = 'zstd'
    #Real version: 0.1.11
    __version__ = '0.1.16'
    ...
    dtype = strax.peak_dtype(n_channels=494)+[(('Labels which cluster the peaklet belongs to','cluster_number'), np.int16)]+[(('Inner-cluster distance parameter health','eta'), np.float32)]+[(('Inner-cluster distance parameter health, no threshold','eta_nothresh'), np.float32)]+[(('Number of clusters detected','kmeans_clusters'), np.int16)]
    ...
    def compute(self, records, start, end):
        ...
        eta_arr = self.calc_etas(peaklets, hits)
        peaklets['eta'] = eta_arr[0]
        peaklets['eta_nothresh'] = eta_arr[1]
        ...
        for peaklet in ds_peaklets:
            ...
            clusters = KMeans(n_clusters=2, random_state=0).fit(arr[hits_used['area'] > (np.sum(hits_used['area'])/self.config['scale_factor'])], sample_weight=hits_used[hits_used['area'] > (np.sum(hits_used['area'])/self.config['scale_factor'])]['area'])
            ...
            #pmt_masks = [pmt_mask, (~(pmt_mask.astype('bool'))).astype('int')]
            for i in [0,1]:
    ...
    def calc_etas(self, peaklets, hits):
        etas_thresh = np.zeros(len(peaklets['time']), dtype=np.float32)
        etas_nothresh = np.zeros(len(peaklets['time']), dtype=np.float32)
        ...
        for p_i, peaklet in enumerate(peaklets):
            if ha_mask[p_i]:
                ...
                hits_used = ds_hits[(ds_hits['channel']<253)]
                alt_hits_used = hits_used[hits_used['area'] > np.sum(hits_used['area'])/self.config['scale_factor']]
                if len(alt_hits_used['time']) <= 2:
                    print("Error found, number of hits: {}".format(len(ds_hits)))
                    etas_thresh[p_i] = float("NaN")
                    etas_nothresh[p_i] = float("NaN")
                    continue
                #start_time = np.min(hits_used['time'])
                #end_time_ind = np.argmax(hits_used['time'])
                #end_time = hits_used['time'][end_time_ind] + hits_used['length'][end_time_ind]*hits_used['dt'][end_time_ind]
                ...
                pmt_pos = np.array(straxen.pmt_positions()[['x','y']])
                top_pmt_pos = pmt_pos[:253]
                hit_positions = pmt_pos[alt_hits_used['channel'][alt_hits_used['channel']<253]]
                hit_positions_nothresh = pmt_pos[hits_used['channel'][hits_used['channel']<253]]

                time_shifted_hits = alt_hits_used[alt_hits_used['channel']<253]['time'] - np.min(alt_hits_used[alt_hits_used['channel']<253]['time'])
                time_shifted_hits_nothresh = hits_used[hits_used['channel']<253]['time'] - np.min(hits_used[hits_used['channel']<253]['time'])
                time_shifted_hits = time_shifted_hits/dt_factor
                time_shifted_hits_nothresh = time_shifted_hits_nothresh/dt_factor

                arr = np.stack((np.array(hit_positions.T[0]), np.array(hit_positions.T[1]), np.array(time_shifted_hits)), axis=-1)
                arr_nothresh = np.stack((np.array(hit_positions_nothresh.T[0]), np.array(hit_positions_nothresh.T[1]), np.array(time_shifted_hits_nothresh)), axis=-1)


                transformed_1 = KMeans(n_clusters=1, random_state=0).fit_transform(arr, sample_weight=alt_hits_used['area'])
                transformed_1_nothresh = KMeans(n_clusters=1, random_state=0).fit_transform(arr_nothresh, sample_weight=hits_used['area'])
                dists_1 = [i[0] for i in transformed_1]
                dists_1_nothresh = [i[0] for i in transformed_1_nothresh]
                inner_distance_1_cluster = np.average(dists_1, weights = alt_hits_used['area'])
                inner_distance_1_cluster_nothresh = np.average(dists_1_nothresh, weights = hits_used['area'])

                two_clusters = KMeans(n_clusters=2, random_state=0).fit(arr, sample_weight=alt_hits_used['area'])
                two_clusters_nothresh = KMeans(n_clusters=2, random_state=0).fit(arr_nothresh, sample_weight=hits_used['area'])

                two_clusters_labels = two_clusters.labels_
                two_clusters_labels_nothresh = two_clusters_nothresh.labels_
                transformed_2 = KMeans(n_clusters=2, random_state=0).fit_transform(arr, sample_weight=alt_hits_used['area'])
                transformed_2_nothresh = KMeans(n_clusters=2, random_state=0).fit_transform(arr_nothresh, sample_weight=hits_used['area'])
                ...
                dists_cluster_1_nothresh = [i[0] for i in transformed_2_nothresh[two_clusters_labels_nothresh == 0]]
                dists_cluster_2_nothresh = [i[1] for i in transformed_2_nothresh[two_clusters_labels_nothresh == 1]]

                inner_distance_2_cluster = np.average(dists_cluster_1, weights = alt_hits_used['area'][two_clusters_labels == 0]) + np.average(dists_cluster_2, weights = alt_hits_used['area'][two_clusters_labels == 1])
                inner_distance_2_cluster_nothresh = np.average(dists_cluster_1_nothresh, weights = hits_used['area'][two_clusters_labels_nothresh == 0]) + np.average(dists_cluster_2_nothresh, weights = hits_used['area'][two_clusters_labels_nothresh == 1])
                etas_thresh[p_i] = (inner_distance_1_cluster - inner_distance_2_cluster)/(inner_distance_1_cluster + inner_distance_2_cluster)
                etas_nothresh[p_i] = (inner_distance_1_cluster_nothresh - inner_distance_2_cluster_nothresh)/(inner_distance_1_cluster_nothresh + inner_distance_2_cluster_nothresh)
        ...
        return np.array([etas_thresh, etas_nothresh])
